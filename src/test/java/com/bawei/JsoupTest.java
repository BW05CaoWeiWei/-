...
public class JsoupTest {
	...
	public void test_xinlang() throws IOException {
		...
		int count = 0;
		// 获取连接对象
		Connection connect = Jsoup.connect("http://www.zanghaihua.org/guichuideng/jingjuegucheng/");
		// 获取文档对象
		...
		for (Element href : ahrefs) {
			...
			String regex = "http://www\\.zanghaihua\\.org/guichuideng/jingjuegucheng/.*html$";
...
//			url.endsWith("html");
			...
			if (url != null && Pattern.matches(regex, url)) {
				...
				// 获取文章的内容元素对象
				Element articleContentElement = articleDoc.getElementById("BookText");
				// 判断元素是否为空
